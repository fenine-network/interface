...
export const generateTweetForAsset = (asset: GenieAsset): string => {
  ...
  }%20(${asset.collectionName})%20https://swap.fene.app/nfts/asset/${asset.address}/${asset.tokenId}%20via%20@fenechain`
}
...
export const generateTweetForPurchase = (assets: UpdatedGenieAsset[], txHashUrl: string): string => {
  ...
  } with @fenechain 🦄\n\nhttps://swap.fene.app/nfts/${collectionUrl}\n${txHashUrl}`
  ...
  const tweetText =
    assets.length == 1
      ? `I just listed ${
          ...
          .join(', ')}. Buy it on Fenswap at https://swap.fene.app/${getAssetHref(assets[0])}`
      : `I just listed ${
          assets.length
        } items on Fenswap at https://swap.fene.app/nfts/profile\n\nCollections: ${mapAssetsToCollections(assets)
